...
class UnitDiscreteController(Controller):
    def __init__(self, env_cfg) -> None:
        """
        added: Auto lichen production if conditions are good!
    ...
    def action_to_lux_action(
        self, agent: str, obs: Dict[str, Any], action: npt.NDArray, step: int
    ):
        shared_obs = obs["player_0"]
        game_state = game_state = obs_to_game_state(step, self.env_cfg, shared_obs)
        ...
        if len(units) == 0:
            ...
            for unit_id, factory in factories.items():
                if factory.can_water(game_state) and factory.cargo.water > 100:
                    lux_action[unit_id] = factory.water()
